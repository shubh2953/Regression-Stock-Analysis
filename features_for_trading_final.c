...
// Updated DataPoint structure
typedef struct {
    double* features;
    double target;
    char ticker_concat[100];    // Combined ticker string (e.g. "AAA-BBB-CCC-DDD")
    char tickers[4][50];        // Individual ticker names: Ticker_A, Ticker_B, Ticker_C, Ticker_D
...
Dataset read_csv(const char* filename) {
    ...
    // Read first data line to determine presence of date and tickers
    ...
    // Count columns and calculate number of features.
    int num_columns = 0;
    ...
    free(header_copy);
    // Subtract: 1 for date, 4 for ticker columns, and 1 for target
    ...
    for (int i = 0; i < total_lines; i++) {
        ...
        // Initialize ticker strings.
        for (int t = 0; t < 4; t++) {
            memset(dataset.data[i].tickers[t], 0, 50);
        }
    }
    ...
    while (fgets(line, MAX_LINE_LENGTH, file) && row < total_lines) {
        ...
        // Store date from the first column.
        strncpy(dataset.data[row].date, token, 10);
        dataset.data[row].date[10] = '\0';
        
        // Process ticker columns (columns 2 to 5).
        char ticker_concat[100] = "";
        for (int t = 0; t < dataset.num_ticker_columns; t++) {
            token = strtok(NULL, ",");
            // Save each individual ticker.
            strncpy(dataset.data[row].tickers[t], token, 49);
            dataset.data[row].tickers[t][49] = '\0';
            // Build combined ticker string with '-' as separator.
            if (t == 0) {
                strcpy(ticker_concat, token);
            } else {
                strcat(ticker_concat, "-");
        ...
        // Append bias term.
        dataset.data[row].features[dataset.num_features] = 1.0;
        free(line_copy);
        row++;
    }
    
    // Replace missing feature values with median values and add EPSILON.
        ...
        for (int j = 0; j < dataset.num_features; j++) {
            if (isnan(dataset.data[i].features[j])) {
                dataset.data[i].features[j] = get_median_for_date(
                    dataset.data, dataset.total_samples, dataset.data[i].date, j
                );
            }
...
// void sliding_window_regression_combined(Dataset dataset, int training_window, const char* combined_output_file, RegressionType reg_type) {
//     // Sort dataset by date descending
//     qsort(dataset.data, dataset.total_samples, sizeof(DataPoint), compare_dates_descending);
    
//     FILE *file = fopen(combined_output_file, "w");
//     if (!file) {
//         printf("Error creating combined output file %s\n", combined_output_file);
//         return;
//     }
    
//     // Write header: include date, each ticker separately, followed by combined ticker.
//     fprintf(file, "Date,Ticker_A,Ticker_B,Ticker_C,Ticker_D,Ticker_Concat");
//     for (int f = 0; f < dataset.num_features; f++) {
//         fprintf(file, ",Target_F%d,Predicted_F%d", f + 1, f + 1);
//     }
//     fprintf(file, "\n");
    
//     // For each data point in the dataset:
//     for (int i = 0; i < dataset.total_samples; i++) {
//         // Obtain a set of training indices from later (historical) rows for the same ticker.
//         int count_training = 0;
//         int *training_indices = malloc(training_window * sizeof(int));
//         for (int j = i + 1; j < dataset.total_samples && count_training < training_window; j++) {
//             if (strcmp(dataset.data[j].ticker_concat, dataset.data[i].ticker_concat) == 0) {
//                 training_indices[count_training++] = j;
//             }
//         }
//         if (count_training < training_window) {
//             free(training_indices);
//             continue;
//         }
        
//         // For every feature, train a one-feature model and compute prediction.
//         double *predictions = malloc(dataset.num_features * sizeof(double));
//         double *targets = malloc(dataset.num_features * sizeof(double));
//         for (int f = 0; f < dataset.num_features; f++) {
//             // Create a temporary training dataset for the current feature.
//             DataPoint *training_data = malloc(training_window * sizeof(DataPoint));
//             for (int k = 0; k < training_window; k++) {
//                 training_data[k].features = malloc(2 * sizeof(double));  // one feature plus bias term
//                 training_data[k].features[0] = dataset.data[training_indices[k]].features[f];
//                 training_data[k].features[1] = 1.0;  // Bias term.
//                 training_data[k].target = dataset.data[training_indices[k]].target;
//             }
            
//             // Allocate weights array (2 parameters: one coefficient and bias).
//             double weights[2];
//             if (reg_type == LINEAR_REGRESSION)
...
//             double current_features[2] = { dataset.data[i].features[f], 1.0 };
//             double pred = predict(current_features, weights, 1, reg_type);
//             predictions[f] = pred;
//             targets[f] = dataset.data[i].target;
            
//             // Free temporary training data.
//             for (int k = 0; k < training_window; k++) {
...
//                 dataset.data[i].date,
//                 dataset.data[i].tickers[0],
//                 dataset.data[i].tickers[1],
//                 dataset.data[i].tickers[2],
//                 dataset.data[i].tickers[3],
//                 dataset.data[i].ticker_concat);
//         for (int f = 0; f < dataset.num_features; f++) {
//             fprintf(file, ",%.4f,%.4f", targets[f], predictions[f]);
//         }
//         fprintf(file, "\n");
...
void sliding_window_regression_combined(Dataset dataset, int training_window, const char* combined_output_file, RegressionType reg_type) {
    ...
    for (int i = 0; i < dataset.total_samples; i++) {
        ...
        double pred_sum = 0.0;
        for (int f = 0; f < dataset.num_features; f++) {
            pred_sum += predictions[f];
        }
        double predicted_avg = pred_sum / dataset.num_features;
        ...
        fprintf(file, ",%.4f\n", predicted_avg);  // Add average prediction
...
int main() {
    ...
    // Read the dataset using the updated read_csv function
    ...
    printf("Running %s regression with training window size: %d\n",
           (reg_type == LINEAR_REGRESSION) ? "linear" : "logistic", training_window);
    ...
    // Run regression to write out the results file as usual.
    sliding_window_regression_combined(dataset, training_window, regression_output_file, reg_type);
    printf("Regression output created in %s\n", regression_output_file);

    // Optionally, generate feature summary here if desired.
    // For example:
    ...
    // ***** The gains chart and NAV metrics generation are skipped *****
    // The following blocks (currently generating gains chart CSV, executing Python for gains chart,
    // calculating NAV metrics, and generating NAV chart) are removed or commented out.
    /*
    ...
    */
    ...
    free_dataset(dataset);
    printf("Regression analysis complete.\n");
    return 0;
}